...
function NavItem({ label, active, count, icon = "•" }: NavItemProps) {
  return (
    <button
      type="button"
...
function MemberItem({ name, detail, accent = "bg-violet-300" }: MemberItemProps) {
  const initials = name
    .split(" ")
    .map((word) => word[0])
    .join("")
...
function ComposerAction({ label }: { label: string }) {
  return (
    <button
      type="button"
